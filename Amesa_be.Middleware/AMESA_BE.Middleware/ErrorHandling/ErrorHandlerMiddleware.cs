...
namespace AMESA_BE.Middleware.ErrorHandling
{
    public class ErrorHandlerMiddleware
    {
        ...
        public ErrorHandlerMiddleware(RequestDelegate next, ILogger<ErrorHandlerMiddleware> logger, IConfiguration configuration, IServiceProvider provider)
        {
            ...
            _provider = provider;
            _restExceptionDetails = configuration.GetValue<bool>("RestExceptionDetails:showExceptionDetails");
        }
                ...
                switch (error)
                {
                    ...
                    case CustomFaultException e:
                        using (var scope = _provider.CreateScope())
                        {
                            ...
                            apiError = new ApiError(e.StatusCode.GetDescription())
                            {
        ...
        private string GetApiVersion()
        {
            ...
            return _configuration.GetSection("Version:Key").Get<string>()!; // READ FROM DOCKER COMPOSE FILE
        }
        ...
        //TODO change UserContext 
        //private static string GetEndPointFromUserContext(HttpContext context)
        //{
        //    return ((UserContext)context.Items["user"]).EndPoints.Where(x => x.Name == "LocalizationService").FirstOrDefault().Url;
